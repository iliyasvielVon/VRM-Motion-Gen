...
Godot 只负责把关键点重定向成骨骼旋转。两种用法共用同一套输出格式：
    ...
    # 实时：摄像头 → UDP 喷给 Godot，模型实时跟着动，Godot 里按「录制」落关键帧
...
def main():
    ...
    src.add_argument("--camera", type=int, help="摄像头编号（实时，一般是 0）")
    ...
    cap = cv2.VideoCapture(args.camera if live else args.video)
    if not cap.isOpened():
        sys.exit("打不开%s：%s" % ("摄像头" if live else "视频", args.camera if live else args.video))
    ...
    while True:
        ...
        if not ok:
            break
        if live:
            bgr = cv2.flip(bgr, 1)   # 摄像头镜像：抬右手，屏幕上的模型也抬右手
    ...
    if args.out:
        ...
        path.write_text(json.dumps({
            "fps": round(fps, 3),
            "source": args.video if not live else "camera%d" % args.camera,
            "frames": frames,
