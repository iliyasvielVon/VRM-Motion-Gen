...
底部时间轴，30fps，最多 240 帧。

1. 点帧格移动播放头，**Shift 点击**拉出区间选择。
2. **F6** 把当前**整身姿势**记成关键帧（金色格）。拖骨头松手、调滑条都会自动落关键帧。
3. 选中一段 → 选缓动曲线 → **「创建补间」**。两端没关键帧会自动补上。
   五种：线性 / 缓入 / 缓出 / 缓入缓出 / 定格。

帧格配色：金 = 关键帧，暗红 = 线性补间，蓝紫 = 缓动补间，灰 = 定格，白边 = 播放头，金边 = 选区。
