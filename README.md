...
- 融合是**各向异性加权**：每台相机沿自己视线方向（深度）只有 0.2 的话语权，图像平面方向
  全额——侧面相机的图像平面正好补正面相机的深度。合成视角实测：单目误差 43mm →
  双目融合 13mm。**正面 + 侧面（45~75°）两台，是性价比最高的摆法**；
- **三层防抖**（多目曾比单目还抖，三个来源各配一味药）：每台相机进门先做一阶低通（压掉
  单机检测抖动再融合）；权重按数据年龄平滑衰减（慢相机两次更新之间逐渐让位，而不是被
  一刀踢出——那个「A 单独 ↔ A+B 混合」的横跳是头号抖源）；输出过 One-Euro 滤波（静止
  死稳、快动作自动放开）。手/表情的来源相机带粘滞（挑战者要明显更好才换人，防闪切）。
  实测：静止满噪声下输出帧间跳动 1.5mm，姿势突变后 1/3 秒追回；
...
- **多路数据融合，抖动的来源在「切换」不在「噪声」。** 多目一开始比单目还抖：慢相机（手机 ~12fps）两次更新之间拿旧帧往回拽、一更新猛跳到新帧；相机跨过新鲜度门槛被整台踢出/放回，输出在「A 单独」和「A+B 混合」两个姿势间横跳（两者因标定偏差本就不同）；手/表情每帧重选来源相机，可见度一抖就闪切。药方全是「把硬边界换成软过渡」：进门低通、年龄权重衰减、来源粘滞、输出 One-Euro。
